...
A modern, fully responsive marketing website for **Innobles Smart Technologies Pvt. Ltd.**, built with React 19, Vite, Tailwind CSS, React Router and Redux Toolkit. All content (services, products, careers, about, contact) is live from the backend API and managed through the `/admin` panel.
...
- **Redux Toolkit** + **React Redux** (auth, products, services, careers, contact, about)
- **TipTap** rich-text editor (admin product descriptions)
...
├── components/
│   ├── about/              # admin About sections (Team/Locations/Statistics)
...
│   ├── Home/               # Hero, ServiceGrid, ProductsHighlight, CTA, Contact
...
├── hooks/                  # useReveal, useScrollToTop, useProducts, …
├── layouts/                # MainLayout (public) + AdminLayout
├── lib/                    # api.js (fetch wrapper) + formValidation.js (shared rules) + richText.js
...
- **State**: each domain has an `Api → Thunks → Slice` trio. Public hooks (`useServices`, `useProducts`, `useAbout`, `useCareers`) fetch once and expose `{ list, status, error }`.
...
Upload the contents of `dist/` to any static host (Vercel, Netlify, Nginx, etc.) and configure SPA fallback so `/admin` and all public routes serve `index.html`. Set `VITE_API_URL` at build time to the deployed backend URL.
