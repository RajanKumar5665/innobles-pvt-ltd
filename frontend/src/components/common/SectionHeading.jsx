...
const SectionHeading = ({ eyebrow, title, subtitle, align = "left", eyebrowClassName = "" }) => {
  const alignment = align === "center" ? "text-center mx-auto" : "text-left";
  const innerAlign = align === "center" ? "justify-center" : "";
  return (
    <Reveal className={`max-w-2xl ${alignment}`}>
      {eyebrow && (
        <p className={`pill-eyebrow mb-4 !text-[#FF9866] ${innerAlign} ${eyebrowClassName}`}>
          {eyebrow}
        </p>
      )}
      <h2 className="font-disp text-3xl font-bold leading-tight md:text-4xl">{title}</h2>
      <span className={`section-heading-underline ${align === "center" ? "" : "section-heading-underline--left"}`} aria-hidden="true" />
