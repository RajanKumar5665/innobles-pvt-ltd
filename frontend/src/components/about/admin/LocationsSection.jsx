import { useState } from "react";
import { ArrowDown, ArrowUp, MapPin, Pencil, Plus, Trash2 } from "lucide-react";
import { api } from "../../../lib/api";
import Loader from "../../common/Loader";
import { DeleteModal, FieldError, FieldLabel, fieldClass, ImageField, Toast } from "./AboutUi";

const emptyForm = () => ({
  city: "",
  country: "",
  address: "",
  phone: "",
  email: "",
  mapLink: "",
  description: "",
});

const emailValid = (value = "") => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value.trim());
const urlSafe = (value = "") => {
  const v = value.trim();
  if (!v) return true;
  try {
    new URL(v);
    return true;
  } catch {
    return false;
  }
};

export const LocationsSection = ({ locations = [], onChanged }) => {
  const [mode, setMode] = useState(null); // null | "create" | item
  const [form, setForm] = useState(emptyForm());
  const [imageFile, setImageFile] = useState(null);
  const [preview, setPreview] = useState("");
  const [imageRemoved, setImageRemoved] = useState(false);
  const [errors, setErrors] = useState({});
  const [busy, setBusy] = useState(false);
  const [formError, setFormError] = useState(null);
  const [deleteTarget, setDeleteTarget] = useState(null);
  const [deleting, setDeleting] = useState(false);
  const [toast, setToast] = useState(null);
  const [reordering, setReordering] = useState(false);

  const resetForm = () => {
    setForm(emptyForm());
    setImageFile(null);
    setPreview("");
    setImageRemoved(false);
    setErrors({});
    setFormError(null);
    setBusy(false);
  };

  const openCreate = () => {
    resetForm();
    setMode("create");
  };

  const openEdit = (item) => {
    setForm({
      city: item.city || "",
      country: item.country || "",
      address: item.address || "",
      phone: item.phone || "",
      email: item.email || "",
      mapLink: item.mapLink || "",
      description: item.description || "",
    });
    setImageFile(null);
    setPreview(item.image?.url || "");
    setImageRemoved(false);
    setErrors({});
    setFormError(null);
    setMode(item);
  };

  const close = () => {
    resetForm();
    setMode(null);
  };

  const showToast = (type, message) => {
    setToast({ type, message });
    window.setTimeout(() => setToast(null), 3500);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (busy) return;

    const errs = {};
    if (!form.city.trim()) errs.city = "Location name is required.";
    if (form.email.trim() && !emailValid(form.email)) errs.email = "Please enter a valid email address.";
    if (form.mapLink.trim() && !urlSafe(form.mapLink)) errs.mapLink = "Please enter a valid URL.";
    setErrors(errs);
    if (Object.values(errs).some(Boolean)) return;

    setBusy(true);
    setFormError(null);
    try {
      const fd = new FormData();
      fd.append("city", form.city.trim());
      fd.append("country", form.country.trim());
      fd.append("address", form.address || "");
      fd.append("phone", form.phone.trim());
      fd.append("email", form.email.trim());
      fd.append("mapLink", form.mapLink.trim());
      fd.append("description", form.description || "");
      if (imageFile) fd.append("image", imageFile);
      if (mode !== "create" && imageRemoved) fd.append("imageRemoved", "true");

      if (mode === "create") await api.postForm("/admin/about/locations", fd);
      else await api.putForm(`/admin/about/locations/${mode._id}`, fd);

      close();
      onChanged();
      showToast("success", mode === "create" ? "Location added." : "Location updated.");
    } catch (err) {
      setFormError(err?.message || "Failed to save the location.");
    } finally {
      setBusy(false);
    }
  };

  const handleDelete = async () => {
    if (!deleteTarget || deleting) return;
    setDeleting(true);
    try {
      await api.delete(`/admin/about/locations/${deleteTarget._id}`);
      setDeleteTarget(null);
      onChanged();
      showToast("success", "Location deleted.");
    } catch (err) {
      showToast("error", err?.message || "Failed to delete the location.");
    } finally {
      setDeleting(false);
    }
  };

  const handleReorder = async (index, dir) => {
    const target = index + dir;
    if (target < 0 || target >= locations.length || reordering) return;
    const ids = locations.map((l) => l._id.toString());
    [ids[index], ids[target]] = [ids[target], ids[index]];
    setReordering(true);
    try {
      await api.patch("/admin/about/locations/reorder", { ids });
      onChanged();
    } catch (err) {
      showToast("error", err?.message || "Failed to reorder locations.");
    } finally {
      setReordering(false);
    }
  };

  return (
    <div className="mt-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-disp text-lg font-bold text-slate-900">Locations</h2>
          <p className="mt-1 text-sm text-slate-500">
            {locations.length} location{locations.length === 1 ? "" : "s"} · reorder with the arrows
          </p>
        </div>
        {!mode && (
          <button type="button" onClick={openCreate} className="btn-primary inline-flex items-center gap-2">
            <Plus size={16} aria-hidden="true" /> Add Location
          </button>
        )}
      </div>

      {mode && (
        <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-start justify-between gap-3">
            <h3 className="font-disp text-lg font-bold text-slate-900">
              {mode === "create" ? "Add Location" : "Edit Location"}
            </h3>
            <button
              type="button"
              onClick={close}
              disabled={busy}
              className="rounded-lg p-1 text-slate-400 hover:bg-slate-100"
              aria-label="Close"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12" /></svg>
            </button>
          </div>

          <div className="grid gap-6 sm:grid-cols-2">
            <ImageField
              id="location-image"
              label="Location image"
              preview={preview}
              onFile={(f) => {
                setImageFile(f);
                setImageRemoved(false);
                setPreview(URL.createObjectURL(f));
              }}
              onRemove={() => {
                setImageFile(null);
                setPreview("");
                setImageRemoved(Boolean(preview && !imageFile));
              }}
              error={errors.image}
            />
            <div>
              <FieldLabel htmlFor="location-city" required>
                Location Name
              </FieldLabel>
              <input
                id="location-city"
                value={form.city}
                onChange={(e) => setForm((f) => ({ ...f, city: e.target.value }))}
                placeholder="Lucknow"
                className={fieldClass(!!errors.city)}
              />
              <FieldError message={errors.city} />
            </div>
          </div>


          <div className="mt-6">
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">Contact Information</p>
            <div className="grid gap-6 sm:grid-cols-2">
              <div>
                <FieldLabel htmlFor="location-phone">Phone</FieldLabel>
                <input
                  id="location-phone"
                  value={form.phone}
                  onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))}
                  placeholder="+91 98765 43210"
                  className={fieldClass(false)}
                />
              </div>
              <div>
                <FieldLabel htmlFor="location-email">Email</FieldLabel>
                <input
                  id="location-email"
                  value={form.email}
                  onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))}
                  placeholder="lucknow@innobles.in"
                  className={fieldClass(!!errors.email)}
                />
                <FieldError message={errors.email} />
              </div>
            </div>
          </div>

          <div className="mt-6">
            <FieldLabel htmlFor="location-address">Address</FieldLabel>
            <textarea
              id="location-address"
              value={form.address}
              onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))}
              rows={3}
              placeholder="2nd Floor, Hazratganj, Lucknow, Uttar Pradesh 226001, India"
              className={fieldClass(false)}
            />
          </div>

          <div className="mt-6 grid gap-6 sm:grid-cols-2">
            <div>
              <FieldLabel htmlFor="location-country">Country</FieldLabel>
              <input
                id="location-country"
                value={form.country}
                onChange={(e) => setForm((f) => ({ ...f, country: e.target.value }))}
                placeholder="India"
                className={fieldClass(false)}
              />
            </div>
            <div>
              <FieldLabel htmlFor="location-mapLink">Map URL (optional)</FieldLabel>
              <input
                id="location-mapLink"
                value={form.mapLink}
                onChange={(e) => setForm((f) => ({ ...f, mapLink: e.target.value }))}
                placeholder="https://maps.app.goo.gl/..."
                className={fieldClass(!!errors.mapLink)}
              />
              <FieldError message={errors.mapLink} />
            </div>
          </div>

          <div className="mt-6">
            <FieldLabel htmlFor="location-description">Short Description (optional)</FieldLabel>
            <textarea
              id="location-description"
              value={form.description}
              onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
              rows={2}
              className={fieldClass(false)}
            />
          </div>

          {formError && <p className="mt-4 text-sm text-red-600">{formError}</p>}

          <div className="mt-6 flex flex-wrap gap-3">
            <button type="submit" disabled={busy} className="btn-primary inline-flex items-center gap-2 disabled:cursor-not-allowed disabled:opacity-60">
              {busy && <Loader size="sm" />} {mode === "create" ? "Save Location" : "Save Changes"}
            </button>
            <button type="button" onClick={close} disabled={busy} className="btn-ghost">
              Cancel
            </button>
          </div>
        </form>
      )}


      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
        {locations.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-brand-orange/10 text-brand-orange">
              <MapPin size={24} strokeWidth={1.5} aria-hidden="true" />
            </span>
            <h3 className="mt-4 font-disp text-lg font-bold text-slate-900">No locations yet</h3>
            <p className="mt-1.5 text-sm text-slate-500">Add your office locations to show up on the About page.</p>
            <button type="button" onClick={openCreate} className="btn-primary mt-6 inline-flex items-center gap-2">
              <Plus size={16} aria-hidden="true" /> Add Location
            </button>
          </div>
        ) : (
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Image</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Location</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Country</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Phone</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {locations.map((loc, i) => (
                <tr key={loc._id} className="hover:bg-slate-50">
                  <td className="px-4 py-3">
                    {loc.image?.url ? (
                      <img src={loc.image.url} alt={loc.city} className="h-10 w-16 rounded-lg object-cover" />
                    ) : (
                      <span className="text-slate-400">—</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-sm font-medium text-slate-900">{loc.city}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{loc.country || "—"}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{loc.phone || "—"}</td>
                  <td className="px-4 py-3 text-right text-sm">
                    <div className="flex items-center justify-end gap-1.5">
                      <button type="button" onClick={() => openEdit(loc)} className="rounded-lg p-1.5 text-brand-cyan hover:bg-slate-100" aria-label="Edit"><Pencil size={14} aria-hidden="true" /></button>
                      <button type="button" onClick={() => setDeleteTarget(loc)} className="rounded-lg p-1.5 text-red-600 hover:bg-red-50" aria-label="Delete"><Trash2 size={14} aria-hidden="true" /></button>
                      <button type="button" onClick={() => handleReorder(i, -1)} disabled={i === 0 || reordering} className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 disabled:opacity-40" aria-label="Move up"><ArrowUp size={14} aria-hidden="true" /></button>
                      <button type="button" onClick={() => handleReorder(i, 1)} disabled={i === locations.length - 1 || reordering} className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 disabled:opacity-40" aria-label="Move down"><ArrowDown size={14} aria-hidden="true" /></button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>

      {deleteTarget && (
        <DeleteModal title={deleteTarget.city} busy={deleting} onCancel={() => setDeleteTarget(null)} onDelete={handleDelete} />
      )}
      <Toast toast={toast} onDismiss={() => setToast(null)} />
    </div>
  );
};

