...
const locationSchema = Joi.object({
  city: Joi.string().trim().min(1).max(200).required().messages({
    ...
    "string.max": "City must be 200 characters or fewer",
  }),
  country: Joi.string().trim().allow("", null).max(200),
  address: Joi.string().allow("", null).max(2000),
  // Office phone lines may be landlines, so a generic length cap is used here
  // instead of the mobile-only phoneString() rule.
